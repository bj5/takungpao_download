...
def get_takungpao_hk_pdfs(today):
    if today is None:
        today = datetime.now().strftime("%Y%m%d")
    base_url = f"http://www.takungpao.com.hk/paper/list-{today}.html"
    ...
    try:
        ...
        for img_tag in soup.find_all('img'):
            ...
            if alt_text.strip():  # 过滤空值
                ...
                if matches:
                    print(matches[0])
                    print(f"https://paper.takungpao.com/resfile/PDF/{today}/PDF/{matches[0].lower()}_screen.pdf")
                    pdf_links.append(f"https://paper.takungpao.com/resfile/PDF/{today}/PDF/{matches[0].lower()}_screen.pdf")
        ...
        if pdf_files:
            ...
            output_filename = f"大公报香港版_{today}.pdf"
